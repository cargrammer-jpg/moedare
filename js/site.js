...
function renderSocials() {
  const navs = document.querySelectorAll("[data-socials]");
  const live = socials.filter((s) => typeof s.url === "string" && s.url.trim() !== "");
...
function loadHero() {
  const well = document.querySelector("[data-hero]");
  if (!well) return;

  const sources = [well.getAttribute("data-hero"), "/car.jpg", "/public/car.jpg"].filter(Boolean);
  ...
  const tryNext = (i) => {
    ...
    const img = new Image();
    img.onload = () => {
      img.alt = "Supercharged BMW Z4 M Coupe";
